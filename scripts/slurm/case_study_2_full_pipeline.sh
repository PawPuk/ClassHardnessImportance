...
oversampling_strategies=('random' 'SMOTE' 'rEDM' 'aEDM' 'hEDM')
case_study_job_ids=()
...
do
    ...
    do
        ...
        if [ "$oversampling_strategy" == "SMOTE" ]; then
            ...
            oversampling_code="r"
        elif [ "$oversampling_strategy" == "rEDM" ]; then
            oversampling_code="rE"
        elif [ "$oversampling_strategy" == "aEDM" ]; then
            oversampling_code="aE"
        else
            oversampling_code="hE"
        fi

        for alpha in "${alphas[@]}"
        do
            job_name="${dataset_code}${oversampling_code}${alpha}"
            log_file="Output/output_case_study_2_${dataset_name}_${oversampling_strategy}_oversampling_alpha_${alpha}.out"

            jid=$(sbatch \
                --dependency=afterok:"$baseline_dependency" \
                --job-name="$job_name" \
                --output="$log_file" \
                scripts/slurm/run_experiments_for_case_study_2.sh \
                "$dataset_name" "$oversampling_strategy" "$alpha" | awk '{print $4}')
...
do
    ...
    job_name="cs2${dataset_code}vis"
    log_file="Output/output_case_study_2_visualization_${dataset_name}.out"

    # sbatch --job-name="$job_name" --output="$log_file" scripts/slurm/visualize_case_study_2_results.sh "$dataset_name"
    sbatch --dependency=afterok:"$case_dependency" \
           --job-name="$job_name" \
           --output="$log_file" \
           scripts/slurm/visualize_case_study_2_results.sh "$dataset_name"
