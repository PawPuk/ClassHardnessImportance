...
def load_models(dataset_name: str) -> Dict[str, Dict[int, List[dict]]]:
...
def compute_pruned_percentage(dataset_name: str, models_by_rate: [str, Dict[int, List[dict]]]) -> Dict[str, Dict[
                                                                                                  int, List[float]]]:
    pruned_percentages = {}
    for pruning_strategy in ['fclp', 'dlp']:
        ...
        for pruning_rate in pruning_rates:
            if pruning_rate != 0:
                ...
                for c in range(num_classes):
                    pruned_percentage = 100.0 * (num_training_samples[c] - remaining_data_count[c]) / \
                                        num_training_samples[c]
                    pruned_percentages[pruning_strategy][pruning_rate].append(pruned_percentage)
            else:
                pruned_percentages[pruning_strategy][0] = [0.0 for _ in range(num_classes)]

    return pruned_percentages


def plot_pruned_percentages(pruned_percentages: Dict[int, List[float]]):
    ...
    plt.savefig(os.path.join(figure_save_dir, f'class_vs_dataset_pruning_values.pdf'))


def evaluate_block(ensemble: List[dict], test_loader, class_index: int, pruning_rate: int, results, pruning_strategy):
...
def evaluate_ensemble(models_by_rate: Dict[str, Dict[int, List[dict]]], test_loader: DataLoader,
    ...
    for pruning_strategy in ['fclp', 'dlp']:
        for pruning_rate, ensemble in tqdm(models_by_rate[pruning_strategy].items(),
            ...
            for metric_name in ['Tp', 'Fp', 'Fn', 'Tn']:
                results[pruning_strategy][metric_name][pruning_rate] = {class_id: 0 for class_id in range(num_classes)}
            for class_index in range(num_classes):
                evaluate_block(ensemble, test_loader, class_index, pruning_rate, results, pruning_strategy)
...
def plot_class_level_results(results, pruned_percentages):
    ...
    for metric_name in results['fclp'].keys():
        ...
        plt.savefig(os.path.join(figure_save_dir, f'Class_Level_{metric_name}.pdf'))
        plt.close()


def compare_fclp_with_dlp(results):
    pruning_rates = sorted(results['fclp']['Average Model Accuracy'].keys())

    for metric_name in results['fclp'].keys():
        avg_metric_fclp = [np.mean([results['fclp'][metric_name][p][class_id]
                                    for class_id in range(num_classes)]) for p in pruning_rates]
        ...
        # Save & Show
        plt.savefig(os.path.join(figure_save_dir, f'{metric_name}fclp_vs_dlp.pdf'))
...
def main(dataset_name):
    ...
    pruned_percentages = compute_pruned_percentage(dataset_name, models)
    plot_pruned_percentages(pruned_percentages['dlp'])
    ...
    if os.path.exists(os.path.join(result_dir, "ensemble_results.pkl")):
        ...
        evaluate_ensemble(models, test_loader, results)
    ...
    print(results['fclp']['Recall'][10])
    plot_class_level_results(results, pruned_percentages)
    compare_fclp_with_dlp(results)
