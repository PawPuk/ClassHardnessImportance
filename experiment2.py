...
from utils import load_aum_results, set_reproducibility


class Experiment2:
    def __init__(self, pruning_strategy, dataset_name, pruning_rate):
        print(dataset_name, pruning_strategy, pruning_rate)
        set_reproducibility()
    ...
    def prune_dataset(self, aum_scores, labels, training_loader):
    ...
    def run_experiment(self):
        ...
        aum_scores = load_aum_results(f"Results/unclean{self.dataset_name}/", self.NUM_EPOCHS)
        labels = np.array([label for _, label, _ in training_dataset])

        # Perform dataset-level pruning
        pruned_dataset = self.prune_dataset(aum_scores, labels, training_loader)
...
if __name__ == "__main__":
    ...
    experiment = Experiment2(args.pruning_strategy, args.dataset_name,  args.pruning_rate)
